...
class MainModel
{
    ...
    public function search($keywords){
        $sql = "SELECT *,MATCH(description) AGAINST(?) AS score FROM products WHERE  MATCH (description) AGAINST (?) ORDER BY score DESC;";
        $this->db->query($sql);
    ...
    public function getCart($user){
        $sql = "SELECT c.id as cart_id, p.* FROM cart c join products p ON c.product_id = p.product_id WHERE c.user = ?";
        $this->db->query($sql);
